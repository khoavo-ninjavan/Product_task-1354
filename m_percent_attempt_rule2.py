...
# rule 2 : Total ít nhất 1 attempt trễ => late_attempt
...
for event,call_log in raw.items():
    ...
    full['is_att_sync_late'] = full.groupby('attempt_datetime')['is_late'].transform('max')
    full = full[['attempt_datetime','is_att_sync_late']].drop_duplicates()

    #if loop != 0:
    raw_attempt = pd.concat([raw_attempt,full])
    soon_attempt += len(full[full.is_att_sync_late == 0])
    late_attempt += len(full[full.is_att_sync_late == 1])
